...
export class CargoTypeService {
  ...
  getById(cargoTypeId: string): Observable<CargoType> {
    return this.httpClient.get<CargoType>(`${environment.dbUrl}/api/cargo-type/${cargoTypeId}.json`);
  }

  update(customer: CargoType): Observable<{ message: string }> {
    return this.httpClient.put<{ message: string }>(`${environment.dbUrl}/api/cargo-type.json`, customer);
  }

  create(customer: CargoType): Observable<CargoType> {
    return this.httpClient.post<CargoType>(`${environment.dbUrl}/api/cargo-type.json`, customer);
  }

  delete(cargoTypeId: number): Observable<{ message: string }> {
    return this.httpClient.delete<{ message: string }>(`${environment.dbUrl}/api/cargo-type/${cargoTypeId}.json`);
  }
}
